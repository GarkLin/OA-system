...
function getGPSLocation(sFlag) {
  if((getCookie("colloaV8.appName")||"").indexOf("ios")>=0) {
    ...
    if(_sGPSLocation == "") {
      ...
      sLatitude=Number(aArr[0])+0.0114874203;
      sLongitude=Number(aArr[1])+0.0069625038;
      _sGPSLocation=sLatitude+","+sLongitude;
      var sLocations=ajax("../addon/webService.aspx?urlRequest="+encodeURIComponent("http://api.map.baidu.com/geocoder/v2/?ak=4fOijRNDeedlWaunRgB4EvctzBZ4YoIg&callback=renderReverse&location="+_sGPSLocation+"&output=xml&pois=1"));
      ...
      return "<a href='#' onclick=\"javaScript:window.open('http://api.map.baidu.com/marker?location="+_sGPSLocation+"&title=Location&content="+sLocation+"&output=html');\"><i class='fa fa-map-marker fa-lg'></i> "+sLocation+"</a>";
    }
  }
  else if((getCookie("colloaV8.appName")||"").indexOf("android")>=0) {
    ...
    if(_sGPSLocation == "") {
      ...
      var aArr=_sGPSLocation.split(',');
      sLatitude=Number(aArr[0])+0.0033334203;
      sLongitude=Number(aArr[1])+0.0112425038;
      _sGPSLocation=sLatitude+","+sLongitude;      
      var sLocations=ajax("../addon/webService.aspx?urlRequest="+encodeURIComponent("http://api.map.baidu.com/geocoder/v2/?ak=4fOijRNDeedlWaunRgB4EvctzBZ4YoIg&callback=renderReverse&location="+_sGPSLocation+"&output=xml&pois=1"));
      var iIndex1=sLocations.indexOf('<formatted_address>');
      var sLocation="";
  ...
  for (var i = 0; i < ec.length; i++) {
    ...
    if (e.id && e.id.indexOf("e.")==0) {
      ...
      if (fieldCheckByForm(ep.getAttribute("dbf.type"), ep.getAttribute("dbf.key")||e.innerHTML, e)==0) {
        if(e.getAttribute("contentEditable") || e.innerHTML.length>0) e.style.backgroundColor = "gold";
        else ep.style.backgroundColor = "gold";
        return 0;
      }
      else e.style.backgroundColor = "";
    }
